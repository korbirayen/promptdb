...
def load_fabric_patterns(patterns_root: Path) -> Iterator[PromptRow]:
    ...
    for pattern_dir in sorted([p for p in patterns_root.iterdir() if p.is_dir()]):
        ...
        yield PromptRow(
            ...
            source_repo=None,
            source_path=str(pattern_dir.relative_to(patterns_root)).replace("\\", "/"),
        )
...
def main() -> int:
    ...
    try:
        ...
        # 1) Fabric patterns from this repo
        patterns_root = root / "data" / "patterns"
        for pr in load_fabric_patterns(patterns_root):
